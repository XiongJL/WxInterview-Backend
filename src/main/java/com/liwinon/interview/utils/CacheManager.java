...
public class CacheManager {   
    ...
    //保存缓存信息   
    public static void putCacheInfo(String key, Cache obj, long dt,boolean expired,boolean isForever) {   
        Cache cache = new Cache();   
        cache.setKey(key);   
        cache.setTimeOut(dt + System.currentTimeMillis()); //设置多久后更新缓存   
        cache.setValue(obj);   
        cache.setExpired(expired); //缓存默认保存时，失效状态为FALSE   
        cache.setForever(isForever); //是否永久
